...
The current repository includes a reference policy server, schemas and examples, deterministic admission/requalification demos, and an optional ROS 2 adapter. The Nav2 work has been validated through real Humble `SpeedLimit` publication, through an unmodified ControllerServer into a test-only controller plugin's official `setSpeedLimit()` method, and with stock Regulated Pure Pursuit command output changing from 1.0 m/s to 0.5 m/s under the same active goal. It has not demonstrated physical speed enforcement or stopping. The local verified-evidence path authenticates configured Ed25519 issuers; the legacy path remains explicitly trusted-local. Review should distinguish what the reference implementation proves from the deployment infrastructure it intentionally leaves external.
...
3. **Evidence and trust.** Where are the evidence and trust assumptions weakest? The reference implementation verifies structural integrity, freshness, binding, replay, sufficiency, and configured Ed25519 issuers, but it does not provide PKI, remote trust discovery, production key custody, or hardware attestation. Which trust relationships require standardization first?
